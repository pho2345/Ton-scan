...
export const checkAddress = function (address) {
    return http.get(`detectAddress?address=${address}`).then(({ data }) => data);
};
...
export const getAddressTransactions = function (address, { limit = 50, offset = 0, end_utime }) {
    return http.get(`getTransactions?address=${address}`, { params: { limit, offset, end_utime, archival: true } }).then(({ data }) => data.result);
};
